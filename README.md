...
- **价格预警（完整闭环）**：订阅时把当前最低的「店铺已审核 + 报价有货」价格保存为基线；供应商提交新报价版本后，只有店铺已审核、有货、相对基线降幅达到订阅比例且新价不高于目标价时才触发站内提醒，同一报价版本重复提交只生成一条记录。个人中心分「进行中 / 已触发」展示原价、现价、降幅、触发时间与当前可采购状态；该商品暂时没有有效报价时标明「暂不可比较」。页面提供「供应商改价」演示入口，可直接提交报价观察触发效果。
...
| GET/POST | `/api/v1/alerts` | 个人中心订阅（进行中/已触发分组，含基线、现价、降幅、触发时间、可采购状态）/ 创建价格预警（保存当前最低有效报价为基线） |
| POST | `/api/v1/supplier/offers` | 供应商提交新报价版本（supplier/admin 角色）；满足条件自动触发预警，同版本幂等 |
| GET | `/api/v1/demo/supplier-token` | 演示用供应商 JWT，供页面「供应商改价」入口调用 |
